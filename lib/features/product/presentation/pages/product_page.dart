    ...
    return Scaffold(
      ...
      body: BlocListener<CartCubit, CartState>(
        listener: (context, state) {
          if (state.addToCartStatus == UsecaseStatus.running) {
            LoadingManager.show();
          }
          if (state.addToCartStatus == UsecaseStatus.error) {
            LoadingManager.hide();
            MessageHelper.showErrorSnackBar(state.addToCartFailure!.response.message);
          }
          if (state.addToCartStatus == UsecaseStatus.completed) {
            LoadingManager.hide();
            MessageHelper.showSuccessSnackBar(state.addToCartResponse.message);
          }
        },
        child: BlocSelector<
            ProductCubit,
          ...
          selector: (state) => (
            ...
            status: state.productDetailsStatus,
            failure: state.productDetailsFailure,
            relatedProducts: state.relatedProducts.data!,
            youMayLikeProducts: state.youMayLikeProducts.data!,
          ),
          builder: (context, state) {
            ...
            return CustomScrollView(
              slivers: [
                ...
                if (state.youMayLikeProducts.isNotEmpty) ...[
                  Padding(
                    padding: REdgeInsets.symmetric(horizontal: 16),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
